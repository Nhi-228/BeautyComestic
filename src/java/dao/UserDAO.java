...
public class UserDAO {
    ...
    public boolean checkLogin(String email, String password) throws SQLException {
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = stmt.executeQuery()) {
                ...
                if (!result) {
                    System.out.println("DEBUG: Không tìm thấy user hoặc tài khoản bị khóa!");
                }
                return result;
            }
        } catch (SQLException e) {
            System.out.println("DEBUG: Lỗi truy vấn SQL: " + e.getMessage());
            throw e;
        }
    }
    ...
    private User extractUser(ResultSet rs) throws SQLException {
        ...
        // user.setVerified(rs.getBoolean("verified")); // Bỏ vì không có trong CSDL
        // user.setBlocked(rs.getBoolean("blocked"));   // Bỏ vì không có trong CSDL
